...
abstract class LazyRepo<TKey, TVal> extends Repo<TKey?, TVal?> {
  ...
  Stream<MapEntry<TKey, TVal>> firstEntryStream([
    ...
    yield await (firstOrNull as FutureOr<MapEntry<TKey, TVal>>);
    yield* dataBox!
        .watch()
        .debounceTime(debounceDuration) //debounce to prevent spamming the event
        .switchMap(((event) => firstOrNull.asStream() as Stream<MapEntry<TKey, TVal>>) as Stream<MapEntry<TKey, TVal>> Function(BoxEvent));
  }
